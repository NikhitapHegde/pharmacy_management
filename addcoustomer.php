...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errors)) {
    ...
    $sqlCustomer = "INSERT INTO customer (id, name, date_of_birth, mobile_number, address)
                     VALUES (?, ?, ?, ?, ?)";
    $stmtCustomer = $conn->prepare($sqlCustomer);

    $sqlMedicine = "INSERT INTO medicine (id, name, qty) VALUES (?, ?, ?)";
    $stmtMedicine = $conn->prepare($sqlMedicine);

    $sqlOtherItem = "INSERT INTO other_item (id, name, qty) VALUES (?, ?, ?)";
    $stmtOtherItem = $conn->prepare($sqlOtherItem);
