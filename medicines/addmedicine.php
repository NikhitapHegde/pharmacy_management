...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Validate medicine name (optional)
  ...
  if (empty($errors)) {
    ...
    $sql = "INSERT INTO medicines (name, qty, price, expiry_date)
             VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);

    // Bind parameters and execute insertion
    $stmt->bind_param("ssss", $medicineName, $quantity, $price, $expiryDate);
    if ($stmt->execute()) {
      echo "Medicine added successfully!"; // Success message
    } else {
